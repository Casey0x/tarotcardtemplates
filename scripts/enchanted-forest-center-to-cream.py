...
INPUT = ROOT / "public" / "images" / "templates" / "enchanted-forest-tarot-border.png"
OUTPUT = ROOT / "public" / "images" / "templates" / "enchanted-forest-tarot-border-cream.png"
...
def fill_center_black_with_cream(path_in: Path = INPUT, path_out: Path = OUTPUT) -> None:
  ...
  # Start from center; interior should be a contiguous dark region
  cx, cy = w // 2, h // 2

  if not is_dark(pixels[cx, cy]):
    # Fallback: search a small neighborhood for a dark seed pixel
    found = None
